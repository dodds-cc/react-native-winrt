...
export class DnssdLookupHelper {
  ...
  private static readonly SERVICE_NAME = "_http._tcp"; // Change as needed. Example: "_ipp._tcp" in "fooDevice._ipp._tcp.local".
  ...
  private static readonly SERVICENAME_PROPERTY =
    "System.Devices.Dnssd.ServiceName";
  private static readonly INSTANCENAME_PROPERTY =
    "System.Devices.Dnssd.InstanceName";
  private static readonly IPADDRESS_PROPERTY = "System.Devices.IpAddress";
  private static readonly PORTNUMBER_PROPERTY =
    "System.Devices.Dnssd.PortNumber";
  private static readonly TEXTATTRIBUTES_PROPERTY =
  ...
  private static propertyKeysEnumerable =
    DnssdLookupHelper.propertyKeys as unknown as Windows.Foundation.Collections.IIterable<string>;
  ...
  private currentSessionCallback: (
    discoveredServices: IDnssdServiceInstance[],
  ) => void = () => {};

  private DEVICE_INFORMATION_PARSING_DELAY: number = 2000;
   ...
   */
  public startListeningForDevicesAsync(
    callback: (discoveredServices: IDnssdServiceInstance[]) => void,
    shouldListenForUpdates: boolean = false,
  ): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      if (!this.DeviceWatcher) {
        ...
        this.DeviceWatcher.addEventListener(
          "enumerationcompleted",
          (_watcher, _comDevice) => {
            console.log(
              `device enumeration completed. watcher status: ${_watcher.status}`,
            );
        ...
        if (shouldListenForUpdates) {
          this.DeviceWatcher.addEventListener(
            "updated",
            this.onDeviceUpdated.bind(this),
          );
        }
  ...
  private async handleDeviceAdded(
    _comDevice: any,
    callback: (discoveredServices: IDnssdServiceInstance[]) => void,
  ): Promise<void> {
  ...
  private async handleDeviceUpdated(
    _comDevice: any,
    callback: (discoveredServices: IDnssdServiceInstance[]) => void,
  ): Promise<void> {
    await delay(this.DEVICE_INFORMATION_PARSING_DELAY);
